...
namespace GearboxDriver.Domain.Tests.DriveModes
{
    public class SportModeTests
    {
        ...
        [Fact]
        public void Accelerate_ShouldDownshiftTwice_WhenStrongKickdown()
        {
            var threshold = new Threshold(0.8d);

            Sut().Accelerate(threshold);

            _gearShifter.Received(2).Downshift();
        }
        ...
        {
            Action act = () => new SportMode(null, _gearShifter, _aggressiveModeFactory);
        ...
        {
            Action act = () => new SportMode(_gearShiftBoundaries, null, _aggressiveModeFactory);
        ...
        {
            Action act = () => new SportMode(_gearShiftBoundaries, _gearShifter, null);

            act.Should().Throw<ArgumentNullException>()
                .Which.ParamName.Should().Be("aggressiveModeFactory");
        }

        private SportMode Sut() => new SportMode(_gearShiftBoundaries, _gearShifter, _aggressiveModeFactory);
    }
}